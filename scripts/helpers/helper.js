...
hexo.extend.helper.register('createNewArchivePosts', function (posts) {
  ...
  posts.forEach(post => postYearList.push(post.updated.year()));
  ...
  postList.forEach(item => {
    posts.forEach(post => item.year === post.updated.year() && item.postList.push(post))
  });
  postList.forEach(item => item.postList.sort((a, b) => b.updated.unix() - a.updated.unix()));
  return postList;
});
